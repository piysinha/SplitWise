...
public class HeapSettleUpStrategy implements SettleUpStrategy{
    ...
    public List<Transaction> settleUp(List<Expense> expensesToSettleUp) {
        List<Transaction> settleUpTransactions = new ArrayList<Transaction>();
        ...
        // else
        // finalAmount[user] -= amount
        Map<User,Integer> finalAmount = new HashMap<User,Integer>();
        for (Expense expense : expensesToSettleUp) {
            for (UserExpense userExpense: expense.getUserExpensesList()){
                User user = userExpense.getUser();
                Integer exisitingAmountOnThisPerson = finalAmount.getOrDefault(user,0);
                if(userExpense.getUserExpenseType() == UserExpenseType.PAID_BY){
                    exisitingAmountOnThisPerson += userExpense.getAmount();
                }else{
                    exisitingAmountOnThisPerson -= userExpense.getAmount();
                }
    ...
    private Transaction getCompletedTransactionFromReverseExpense(Expense expense) {
        ...
        completedTransaction.setRecevingUser(expense.getUserExpensesList().stream().filter(userExpense -> userExpense.getUserExpenseType() == UserExpenseType.PAID_BY).findFirst().get().getUser());
        completedTransaction.setPayingUser(expense.getUserExpensesList().stream().filter(userExpense -> userExpense.getUserExpenseType() == UserExpenseType.HAD_TO_PAY).findFirst().get().getUser());
